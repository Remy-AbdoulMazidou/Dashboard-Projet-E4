...
# ─── Helpers : nouvelles visualisations ──────────────────────────────────────

def _rose_diagram(df, col="orientation_psi", n_bins=18):
    """Rosace des orientations azimutales ψ par matériau."""
    if df.empty or col not in df.columns or "material" not in df.columns:
        return _empty_fig(f"Colonne '{col}' non disponible")
    fig = go.Figure()
    bin_edges = np.linspace(0, 360, n_bins + 1)
    bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
    bin_width = 360 / n_bins
    for i, mat in enumerate(sorted(df["material"].dropna().unique())):
        arr = df[df["material"] == mat][col].dropna().values % 360
        if len(arr) < 3:
            continue
        ...
        fig.add_trace(go.Barpolar(
            r=freq, theta=bin_centers, width=[bin_width] * n_bins,
            name=mat,
            marker=dict(color=color, opacity=0.75, line=dict(color="white", width=0.6)),
            hovertemplate=f"<b>{mat}</b><br>ψ ≈ %{{theta:.0f}}°<br>Fréq. = %{{r:.3f}}<extra></extra>",
        ))
    fig.update_layout(
        paper_bgcolor="white",
        font=dict(family="Inter, system-ui, sans-serif", size=11, color="#334155"),
        polar=dict(
            angularaxis=dict(tickfont=dict(size=10), direction="clockwise", linecolor="#CBD5E1"),
            radialaxis=dict(visible=False, gridcolor="#CBD5E1"),
            bgcolor="#F8FAFC",
        ),
        showlegend=False,
...
def _pole_figure(df, theta_col="orientation_theta", psi_col="orientation_psi"):
    """Figure de pôle — projection stéréographique : r=θ, theta=ψ."""
    if df.empty or not _has(df, theta_col, psi_col, "material"):
        return _empty_fig("Colonnes d'orientation non disponibles")
    if len(df) > 3000:
        df = df.sample(3000, random_state=42)
    fig = go.Figure()
    for i, mat in enumerate(sorted(df["material"].dropna().unique())):
        sub = df[df["material"] == mat].dropna(subset=[theta_col, psi_col])
        if sub.empty:
            continue
        color = mat_color(mat, i)
        size_arr = 4
        if "length_um" in sub.columns:
            lnorm = sub["length_um"].clip(lower=0)
            mx = lnorm.max()
            size_arr = ((lnorm / mx * 8 + 2) if mx > 0 else lnorm * 0 + 4).fillna(4).tolist()
        fig.add_trace(go.Scatterpolar(
            r=sub[theta_col], theta=sub[psi_col],
            mode="markers", name=mat,
            marker=dict(color=color, size=size_arr, opacity=0.55,
    ...
    fig.update_layout(
        ...
        polar=dict(
            ...
            angularaxis=dict(tickfont=dict(size=10, color="#334155"),
                             direction="clockwise", linecolor="#CBD5E1"),
            bgcolor="#F8FAFC",
        ),
        showlegend=False,
        margin=dict(l=60, r=60, t=40, b=40),
...
def _scatter_lxd(df):
    """Scatter longueur × diamètre, un point par fibre, coloré par matériau."""
    if df.empty or not _has(df, "diameter_um", "length_um", "material"):
        return _empty_fig("Colonnes 'diameter_um' ou 'length_um' non disponibles")
    plot_df = df.dropna(subset=["diameter_um", "length_um", "material"])
    if len(plot_df) > 2000:
        plot_df = plot_df.sample(2000, random_state=42)
    fig = go.Figure()
    for i, mat in enumerate(sorted(plot_df["material"].dropna().unique())):
        sub = plot_df[plot_df["material"] == mat]
        fig.add_trace(go.Scatter(
            x=sub["diameter_um"], y=sub["length_um"],
            mode="markers", name=mat,
            marker=dict(color=mat_color(mat, i), size=5, opacity=0.55,
                        line=dict(color="white", width=0.4)),
            hovertemplate=f"<b>{mat}</b><br>Ø = %{{x:.1f}} µm<br>L = %{{y:.1f}} µm<extra></extra>",
        ))
    fig.update_layout(**PLOT_LAYOUT, xaxis_title="Diamètre (µm)", yaxis_title="Longueur (µm)",
                      showlegend=False)
    return apply_grid(fig)


def _bar_slenderness(df):
    """Rapport d'élancement moyen λ = longueur/diamètre par matériau."""
    if df.empty or not _has(df, "diameter_um", "length_um", "material"):
        return _empty_fig("Données manquantes")
    df2 = df.copy()
    df2["slenderness"] = df2["length_um"] / df2["diameter_um"].clip(lower=0.1)
    sl_df = (df2.groupby("material")["slenderness"].mean()
             .reset_index().sort_values("slenderness", ascending=False))
    fig = go.Figure()
    for _, row in sl_df.iterrows():
        mat = row["material"]
        fig.add_trace(go.Bar(
            x=[mat], y=[row["slenderness"]], name=mat,
            marker_color=mat_color(mat),
            hovertemplate=f"<b>{mat}</b><br>λ = %{{y:.0f}}<extra></extra>",
        ))
    fig.update_layout(**PLOT_LAYOUT, xaxis_title="Matériau", yaxis_title="λ = longueur / diamètre",
                      showlegend=False)
    return apply_grid(fig)
                        ...
                        children=[html.Div(style={"padding": "28px 0 12px 0"}, children=[
                            ...
                            dbc.Row(className="px-1 g-3", children=[
                                graph_card(
                                    ...
                                    "Un réseau isotrope donne une rosace uniforme. Un lobe indique une orientation préférentielle.",
                                    "Secteurs larges = de nombreuses fibres pointent dans cette direction. "
                                    "Rosace symétrique = fibres orientées aléatoirement (isotrope).",
                                    height="280px", col_width=4,
                                    accent=TABS["morphology"]["bg"],
                                ),
                                graph_card(
                                    "graph-pole-figure",
                                    "Figure de pôle — projection stéréographique 3D",
                                    "Chaque point = une fibre. Le rayon = θ (0° = ⊥ au plan, 90° = dans le plan). "
                                    "L'angle angulaire = ψ. Taille des points ∝ longueur de la fibre.",
                                    "Points au centre = fibres perpendiculaires au plan. "
                                    "Points en périphérie = fibres couchées. Distribution uniforme = réseau isotrope.",
                                    height="280px", col_width=8,
                                    accent=TABS["morphology"]["bg"],
                                ),
                            ]),
                            dbc.Row(className="px-1 g-3", children=[
                                graph_card(
                                    ...
                                    "Chaque point est une fibre individuelle. Révèle la polydispersité réelle "
                                    "et les populations distinctes par matériau.",
                                    "Nuage compact = fibres uniformes. Nuage étalé = grande variabilité. "
                                    "Chaque couleur = un matériau. Survolez pour voir les valeurs.",
                                    height="280px", col_width=8,
                                    accent=TABS["morphology"]["bg"],
                                ),
                                graph_card(
                                    ...
                                    "Un λ élevé = fibres longues et fines, réseau dense. "
                                    "Corrélé à la perméabilité thermique du matériau.",
                                    "Grand λ = fibres fines et longues (favorable à l'absorption). "
                                    "Petit λ = fibres courtes et épaisses (moins interconnectées).",
                                    height="280px", col_width=4,
                                    accent=TABS["morphology"]["bg"],
                                ),
                            ]),
...
@app.callback(
    ...
    Output("graph-rose-psi",         "figure"),
    Output("graph-pole-figure",      "figure"),
    Output("graph-scatter-lxd",      "figure"),
...
def update_all(vis_mats, bat_sel):
    ...
    # ── Rosace orientation ψ ─────────────────────────────────────────────
    fig_rose_psi = _rose_diagram(fib_f, col="orientation_psi")

    # ── Figure de pôle (projection stéréographique) ───────────────────────
    fig_pole = _pole_figure(fib_f, theta_col="orientation_theta", psi_col="orientation_psi")

    # ── Scatter longueur × diamètre ───────────────────────────────────────
    fig_scatter_lxd = _scatter_lxd(fib_f)

    # ── Rapport d'élancement moyen ────────────────────────────────────────
    fig_slenderness = _bar_slenderness(fib_f)
    ...
    return (kpis, fig_diam, fig_len, fig_ori, fig_curv, fig_cv,
            ...
            fig_rose_psi, fig_pole, fig_scatter_lxd, fig_slenderness)
